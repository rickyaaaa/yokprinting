...
class GenerateProfitLossSpreadsheet
{
    ...
    private function worksheet(array $report): string
    {
        ...
        $rows = [
            ...
            $this->row(16, [$this->textCell('A16', 'Total Biaya Ekspedisi'), $this->numberCell('B16', $summary['expedition_expenses'], 4)]),
            $this->row(17, [$this->textCell('A17', 'Total Biaya Produksi'), $this->numberCell('B17', $summary['production_expenses'], 4)]),
            $this->row(18, [$this->textCell('A18', 'Total Biaya Karyawan'), $this->numberCell('B18', $summary['employee_expenses'], 4)]),
            $this->row(19, [$this->textCell('A19', 'Total Biaya Tempat'), $this->numberCell('B19', $summary['premises_expenses'], 4)]),
            $this->row(20, [$this->textCell('A20', 'Total Biaya Operasional'), $this->numberCell('B20', $summary['operational_expenses'], 4)]),
            $this->row(21, [$this->textCell('A21', 'Total Biaya Admin Bank'), $this->numberCell('B21', $summary['bank_fee_expenses'], 4)]),
            $this->row(22, [$this->textCell('A22', 'Total Belanjaan'), $this->numberCell('B22', $summary['shopping_expenses'], 4)]),
            $this->row(23, [$this->textCell('A23', 'Pengeluaran belum diklasifikasikan terhadap HPP'), $this->numberCell('B23', $summary['unclassified_expenses'], 4)]),
            $this->row(24, [$this->textCell('A24', 'Total Pengeluaran Tercatat', 3), $this->numberCell('B24', $summary['recorded_expenses'], 5)]),
            $this->row(25, [$this->textCell('A25', 'Pengeluaran Diakui di Laba Rugi', 3), $this->numberCell('B25', $summary['recognized_expenses'], 5)]),
            $this->row(26, [$this->textCell('A26', 'Laba Bersih Minimum', 3), $this->numberCell('B26', $summary['net_profit_minimum'], 5)]),
            $this->row(27, [$this->textCell('A27', 'Laba Bersih Maksimum', 3), $this->numberCell('B27', $summary['net_profit_maximum'], 5)]),
            $this->row(28, [$this->textCell('A28', 'Qty Penjualan'), $this->numberCell('B28', $summary['sales_quantity'], 7)]),
            $this->row(29, [$this->textCell('A29', 'Jumlah Invoice'), $this->numberCell('B29', $summary['invoice_count'], 8)]),
            $this->row(30, [$this->textCell('A30', 'Jumlah Transaksi Pengeluaran'), $this->numberCell('B30', $summary['expense_count'], 8)]),
            $this->row(31, [$this->textCell('A31', 'Selisih rekonsiliasi invoice'), $this->numberCell('B31', $summary['invoice_reconciliation_difference'], 4)]),
            $this->row(32, [$this->textCell('A32', 'Selisih rekonsiliasi laba minimum'), $this->numberCell('B32', $summary['minimum_profit_reconciliation_difference'], 4)]),
            $this->row(33, [$this->textCell('A33', 'Selisih rekonsiliasi laba maksimum'), $this->numberCell('B33', $summary['maximum_profit_reconciliation_difference'], 4)]),
            $this->row(35, [
                $this->textCell('A35', 'Keputusan bisnis diperlukan', 6),
                $this->textCell('B35', $report['accounting_policy']['decision_required'], 6),
            ]),
            $this->row(36, [$this->textCell('A36', 'Basis laba minimum', 6), $this->textCell('B36', $report['accounting_policy']['minimum_profit_basis'], 6)]),
            $this->row(37, [$this->textCell('A37', 'Basis laba maksimum', 6), $this->textCell('B37', $report['accounting_policy']['maximum_profit_basis'], 6)]),
        ];
