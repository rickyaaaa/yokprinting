...
class GenerateProfitLossSpreadsheet
{
    ...
    private function worksheet(array $report): string
    {
        ...
        $rows = [
            ...
            $this->row(20, [$this->textCell('A20', 'Total Belanjaan'), $this->numberCell('B20', $summary['shopping_expenses'], 4)]),
            $this->row(21, [$this->textCell('A21', 'Pengeluaran belum diklasifikasikan terhadap HPP'), $this->numberCell('B21', $summary['unclassified_expenses'], 4)]),
            $this->row(22, [$this->textCell('A22', 'Total Pengeluaran Tercatat', 3), $this->numberCell('B22', $summary['recorded_expenses'], 5)]),
            $this->row(23, [$this->textCell('A23', 'Pengeluaran Diakui di Laba Rugi', 3), $this->numberCell('B23', $summary['recognized_expenses'], 5)]),
            $this->row(24, [$this->textCell('A24', 'Laba Bersih Minimum', 3), $this->numberCell('B24', $summary['net_profit_minimum'], 5)]),
            $this->row(25, [$this->textCell('A25', 'Laba Bersih Maksimum', 3), $this->numberCell('B25', $summary['net_profit_maximum'], 5)]),
            $this->row(26, [$this->textCell('A26', 'Qty Penjualan'), $this->numberCell('B26', $summary['sales_quantity'], 7)]),
            $this->row(27, [$this->textCell('A27', 'Jumlah Invoice'), $this->numberCell('B27', $summary['invoice_count'], 8)]),
            $this->row(28, [$this->textCell('A28', 'Jumlah Transaksi Pengeluaran'), $this->numberCell('B28', $summary['expense_count'], 8)]),
            $this->row(29, [$this->textCell('A29', 'Selisih rekonsiliasi invoice'), $this->numberCell('B29', $summary['invoice_reconciliation_difference'], 4)]),
            $this->row(30, [$this->textCell('A30', 'Selisih rekonsiliasi laba minimum'), $this->numberCell('B30', $summary['minimum_profit_reconciliation_difference'], 4)]),
            $this->row(31, [$this->textCell('A31', 'Selisih rekonsiliasi laba maksimum'), $this->numberCell('B31', $summary['maximum_profit_reconciliation_difference'], 4)]),
            $this->row(33, [
                $this->textCell('A33', 'Keputusan bisnis diperlukan', 6),
                $this->textCell('B33', $report['accounting_policy']['decision_required'], 6),
            ]),
            $this->row(34, [$this->textCell('A34', 'Basis laba minimum', 6), $this->textCell('B34', $report['accounting_policy']['minimum_profit_basis'], 6)]),
            $this->row(35, [$this->textCell('A35', 'Basis laba maksimum', 6), $this->textCell('B35', $report['accounting_policy']['maximum_profit_basis'], 6)]),
        ];
