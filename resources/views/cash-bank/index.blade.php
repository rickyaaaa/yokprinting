        ...
        <div class="min-h-screen lg:flex" x-data="{ sidebarOpen: false }" @keydown.escape.window="sidebarOpen = false">
            ...
            <div class="min-w-0 flex-1">
                <header class="sticky top-0 z-20 flex h-16 items-center border-b border-line bg-white/95 px-4 backdrop-blur-sm sm:px-6 lg:px-8">
                    <button type="button" class="btn-icon mr-3 min-h-11 min-w-11 border-transparent lg:hidden" @click="sidebarOpen = true" aria-label="Buka navigasi">
                ...
                <main class="mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8" x-data='cashBankPage(@json($config))' x-init="init()">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button x-show="config.canExport" type="button" class="btn btn-outline min-h-11 disabled:cursor-wait" :disabled="exporting" @click="exportCsv()">
                                <span x-text="exporting ? 'Mengekspor...' : 'Export CSV'"></span>
                            </button>
                            <button x-show="config.canUpdate" type="button" class="btn btn-outline min-h-11 disabled:cursor-not-allowed" :disabled="summaryLoading || !summaryLoaded" :title="summaryLoaded ? 'Ubah rekening utama' : 'Ringkasan rekening belum tersedia'" @click="openAccountSettings()">Atur Rekening</button>
                            <button x-show="config.canCreate" type="button" class="btn min-h-11 border-green-200 bg-green-50 text-green-800 hover:bg-green-100" @click="openCreate('income')">
                                <i class="iconify tabler--plus text-base"></i>
                                Tambah Uang Masuk
                            </button>
                            <button x-show="config.canCreate" type="button" class="btn btn-primary min-h-11" @click="openCreate('expense')">
                                <i class="iconify tabler--plus text-base"></i>
                                Tambah Uang Keluar
                            </button>
                        </div>
                    ...
                    <section x-ref="accountForm" x-cloak x-show="accountSettingsOpen" class="mt-6 card" aria-labelledby="bank-account-heading">
                        <div class="flex items-start justify-between gap-4 border-b border-line px-5 py-4 sm:px-6">
                            ...
                            <button type="button" class="btn-icon min-h-11 min-w-11 border-transparent" @click="accountSettingsOpen = false">
                        ...
                        <form class="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:p-6" @submit.prevent="saveAccount()">
                            ...
                            <button type="submit" class="btn btn-primary min-h-11 self-end disabled:cursor-wait sm:col-span-2 lg:col-span-1" :disabled="savingAccount" x-text="savingAccount ? 'Menyimpan...' : 'Simpan Rekening'"></button>
                        </form>
                    </section>

                    <section x-ref="manualForm" x-cloak x-show="formOpen" class="mt-6 card" aria-labelledby="manual-transaction-heading">
                        <div class="flex items-start justify-between gap-4 border-b border-line px-5 py-4 sm:px-6">
                            <div>
                            ...
                            <button type="button" class="btn-icon min-h-11 min-w-11 border-transparent" @click="formOpen = false">
                                <i class="iconify tabler--x text-base"></i>
                            </button>
                        </div>
                        <form class="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:p-6" @submit.prevent="save()">
                            ...
                            <button type="submit" class="btn btn-primary min-h-11 self-end disabled:cursor-wait" :disabled="saving" x-text="saving ? 'Menyimpan...' : 'Simpan'"></button>
                        </form>
                    </section>
                    ...
                    <section class="mt-6 card" aria-labelledby="cash-bank-history-heading">
                                ...
                                <tbody class="divide-y divide-line">
                                    <template x-for="transaction in transactions" :key="transaction.id">
                                        <tr class="align-top" :class="transaction.status === 'cancelled' ? 'bg-canvas opacity-70' : 'hover:bg-brand-50/40'">
                                            ...
                                            <td class="px-5 py-4">
                                                <span class="badge" :class="transaction.payment_method === 'cash' ? 'badge-warning' : 'badge-brand'" x-text="transaction.payment_method_label"></span>
                                            </td>
                                            ...
                                            <td class="px-5 py-4"><span class="badge" :class="transaction.status === 'posted' ? 'badge-success' : 'badge-default'" x-text="transaction.status_label"></span></td>
                                            <td class="px-5 py-4 text-right sm:px-6"><div class="flex justify-end gap-2" x-show="transaction.is_manual && transaction.status === 'posted'">
                                                <button x-show="config.canUpdate" type="button" class="btn btn-sm btn-outline min-h-11" @click="edit(transaction)">Edit</button>
                                                <button x-show="config.canDelete" type="button" class="btn btn-sm btn-danger-outline min-h-11" @click="cancel(transaction)">Batalkan</button>
                                            </div></td>
                                        </tr>
                                    </template>
                        ...
                        <footer class="flex flex-col gap-3 border-t border-line px-5 py-4 text-sm sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex items-center gap-2"><button type="button" class="btn btn-outline min-h-11" :disabled="loading || meta.current_page <= 1" @click="loadTransactions(meta.current_page - 1)">Sebelumnya</button><span class="text-muted" x-text="`Halaman ${meta.current_page} dari ${meta.last_page}`"></span><button type="button" class="btn btn-outline min-h-11" :disabled="loading || meta.current_page >= meta.last_page" @click="loadTransactions(meta.current_page + 1)">Berikutnya</button></div>
                        </footer>
                    </section>
                </main>
            </div>
